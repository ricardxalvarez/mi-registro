...
function GradeRow({
    ...
    async function onChangeGrade(e: ChangeEvent<HTMLInputElement>) {
        ...
        const updatedPeriods = periods.map((p) => {
          if (p.id === period.id) {
            const updatedGrades = p.grades.some(g => g.student_id === student.id) ? p.grades.map((g) =>
    ...
    const calif_cp4 = ((calif?.rcp4) || (calif?.cp4 || 0))
    return (
        <tr>
            ...
            >
                {
                    ...
                    calif.cp3 &&
                    calif.cp4 ?
                    <>{(calif_cp1 + calif_cp2 + calif_cp3 + calif_cp4) / 4}</>
                    : <></>
                }
            </td>
        </tr>
